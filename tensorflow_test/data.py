...
class ExtractData():
 def __init__(self):
    datafilepath = join(FLAGS.data_dir, FLAGS.datafilename)
    labelfilepath = join( FLAGS.data_dir, FLAGS.labelfilename)
 ...
 def fetch_data(self):
  for data_row in self.data_csvfile:
   if len(self.data_rows) == self.num_examples:
    self.data_rows = numpy.array(self.data_rows)
    self.data_rows.astype(numpy.float32)
    yield self.data_rows
    self.data_rows = []
   else:
    self.data_rows.append(data_row)
 def fetch_label(self):
  for label_row in self.label_csvfile:
   if len(self.label_rows) == self.num_examples:
    self.label_rows = numpy.array(self.label_rows)
    self.label_rows.astype(numpy.int16)
    self.label_rows.reshape(self.num_examples)
    yield self.label_rows
    label_rows = []
   else:
    self.label_rows.append(label_row[1])   
